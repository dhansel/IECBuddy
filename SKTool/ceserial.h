...
inline long ceSerial::Open() {
	...
	else if (parity == 'O') dcb.Parity = ODDPARITY;
	else if (parity == 'M') dcb.Parity = MARKPARITY;
	else if (parity == 'S') dcb.Parity = SPACEPARITY;
    else dcb.Parity = NOPARITY;

    dcb.ByteSize = (BYTE)dsize;
...
inline bool ceSerial::Write(const char *data) {
  return Write(data, (long) strlen(data));
}
...
#else
        fRes = WriteFile(hComm, data, n, &dwWritten, NULL);
#endif
	return fRes;
}
...
inline bool ceSerial::Read(char *data,long n) {
#ifdef ASYNCHRONOUS_IO
  return false;
#else
  DWORD dwRead;
  return ReadFile(hComm, data, n, &dwRead, NULL) && dwRead==n;
#endif
}
...
#else
        success = ReadFile(hComm, data, length, &dwRead, NULL) && dwRead==length;
#endif
